...
function ToggleSwitch({ id, label, value = false, onChange, onBlur }: Props) {
  return (
    <div className="grid grid-cols-[75%,auto] h-14  items-center gap-4  capitalize border-b py-2">
      <span className="text-lg leading-4">{label}</span>
      <label htmlFor={id} className="relative inline-flex items-center cursor-pointer">
        <input type="checkbox" name={id} id={id} checked={value} className="sr-only peer" onChange={onChange} onBlur={onBlur} />
