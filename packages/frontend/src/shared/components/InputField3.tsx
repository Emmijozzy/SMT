  ...
  return (
    <div className={`grid w-full min-h-12 items-center border-b-[1px] border-base-content/40 ${bodyClassName || ""}`}>
      <label htmlFor={name} className="relative flex items-center gap-2">
        <p className="w-fit md:w-fit md:min-w-24 text-sm mt-3 whitespace-nowrap font-bold capitalize mr-1">{label}: </p>
        {error && (
          <span className="absolute top-0 right-0 mt-3 mr-1 label-text-alt text-[10px] text-error">{error}</span>
        )}
        {inputType === "input" && (
          <input
            name={name}
